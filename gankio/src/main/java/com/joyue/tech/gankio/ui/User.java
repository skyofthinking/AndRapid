...
public class User {
    ...
    public User(JSONObject json) throws Exception {
        if (json.has(JSON_ID)) {
            ...
            // 解密后存放
            mId = Aes.decrypt(masterPassword, id);
            mPwd = Aes.decrypt(masterPassword, pwd);
        }
    }

    public JSONObject toJSON() throws Exception {
        // 使用AES加密算法加密后保存
        String id = Aes.encrypt(masterPassword, mId);
        String pwd = Aes.encrypt(masterPassword, mPwd);

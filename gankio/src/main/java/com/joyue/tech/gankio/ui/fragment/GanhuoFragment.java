...
public class GanhuoFragment extends RapidTabFragment {
    ...
    public FragmentPagerItems onSetupTabs() {
        String[] categories = new String[]{"all", "Android", "iOS", "休息视频", "拓展资源", "前端", "瞎推荐", "App"};

        FragmentPagerItems mPages = FragmentPagerItems.with(mContext).create();
        for (int i = 0; i < categories.length; i++) {
            Bundle bundle = new Bundle();
            bundle.putString("category", categories[i]);
            mPages.add(FragmentPagerItem.of(categories[i], GanhuoTabFragment.class, bundle));
        }
